...
def genscat(roidim: np.ndarray, meandist: np.ndarray ,I: Union[np.ndarray, None]  = None, g: Union[np.ndarray, float] = None)  -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Generate a pseudorandom distribution of scatterers.

    Parameters
    ----------
    roidim : array-like
        1-D vector of length 2 or 3 specifying ROI dimensions (m):

        - ``[WIDTH, HEIGHT]`` — 2-D distribution in the x-z plane (y=0).
          Origin is at the middle of the lower edge (x=0, z=0).
        - ``[WIDTH, HEIGHT, DEPTH]`` — 3-D distribution. Origin is at the
          center of the lower face.

        Any entry may be ``NaN`` when *I* is provided; the missing dimension
        is inferred from the image aspect ratio.
    meandist : float or utils.Param
        Mean nearest-neighbour distance between scatterers (m).  Pass a
        :class:`~pymust.utils.Param` structure to use the minimum wavelength
        at -6 dB automatically (requires ``param.fc`` and optionally
        ``param.bandwidth`` and ``param.c``).
    I : np.ndarray, optional
        2-D or 3-D reference image (non-negative values) that modulates the
        reflection coefficients. ``None`` (default) produces reflection
        coefficients drawn from a Rayleigh distribution with unit mean.
    g : float, optional
        Dynamic range of *I* in dB (default 40 dB).  If ``g <= 1``, *I* is
        treated as gamma-compressed with exponent *g*.

    Returns
    -------
    xs : np.ndarray
        x-coordinates of the scatterers (m).
    ys : np.ndarray
        y-coordinates of the scatterers (m); all zeros for a 2-D ROI.
    zs : np.ndarray
        z-coordinates of the scatterers (m).
    RC : np.ndarray
        Reflection coefficients (dimensionless).

    Examples
    --------
    Cardiac scatterers from a reference image:

    >>> from PIL import Image
    >>> import numpy as np
    >>> from pymust import genscat, getparam
    >>> I = np.array(Image.open('heart.jpg').convert('L'))
    >>> param = getparam('P4-2v')
    >>> xs, ys, zs, RC = genscat([np.nan, 15e-2], param, I)

    Notes
    -----
    Translated from the MATLAB MUST toolbox (Damien Garcia, 2021–2022).
