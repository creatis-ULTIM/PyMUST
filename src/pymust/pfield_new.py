...
def pfield_new(x: np.ndarray, y: np.ndarray, z: np.ndarray, delaysTX: np.ndarray, param: utils.Param, isQuick: bool = False, options: utils.Options = None):
...
#
    ...
    if param.xdcr.nelements is not None:
        assert param.xdcr.nelements==NumberOfElements, 'DELAYS must be of length PARAM.Nelements.'
    param.xdcr.nelements = NumberOfElements
    ...
    NonRigidBaffle = param.xdcr.non_rigid_baffle
    ...
    if utils.isfield(options,'ElementSplitting') and not options.ElementSplitting is None:
        ...
        LambdaMin = c/(fc*(1+param.xdcr.bandwidth/200))
    ...
    #%-- FREQUENCY SAMPLES
    Nf = int(2*np.ceil(param.xdcr.fc/df)+1) # number of frequency samples
    f = np.linspace(0,2*param.xdcr.fc,Nf) # frequency samples
    ...
    #%   [Th = angle relative to the element normal axis]
    if NonRigidBaffle:
        if param.xdcr.baffle == 'soft':
            ObliFac = np.cos(Th)
        else: # % param.xdcr.baffle is a scalar
            ObliFac = np.cos(Th)/(np.cos(Th)+param.xdcr.baffle)
   ...
   # %-- EXPONENTIAL arrays of size [numel(x) NumberOfElements M]
    ...
    #%-- TX apodization
    APOD = param.tx.apodization.flatten()
...
#                print(np.linalg.norm(EXP_RC), np.linalg.norm(RPk[nx:]), np.linalg.norm(options.RC))
            ...
            if np.any(param.rx.delay != 0): # % reception delays, if any
                SPECT[k,:] = SPECT[k,:] *np.exp(1j*kw*c*param.rx.delay)
