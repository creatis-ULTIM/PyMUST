...
#GB TODO: this goes 3x slower than matlab. Check why
...
#
    ...
    #%---
    r = np.sqrt(d2+y.reshape((-1,1,1))**2)
   ...
   # %-- EXPONENTIAL arrays of size [numel(x) NumberOfElements M]
    ...
    for k  in range(nSampling):
        ...
        if isFFD: #% isFFD = true -> frequency-dependent directivity
            ...
            RPmono = np.mean(DIR*EXP,2) #; % summation over the M small segments
            # Code added by GB: Doppler effect
            if 'velocity' in options:
                f_shift = (1 + options.velocity.flatten()/param.c) 
            ...
            if M>1:            
                RPmono = np.mean(EXP,2); #% summation over the M small segments
            else:
                RPmono = EXP
